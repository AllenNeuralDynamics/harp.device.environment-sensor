...
Adafruit_BME680::Adafruit_BME680(spi_inst_t* spi_hw,
           uint8_t spi_pico_pin, uint8_t spi_poci_pin, uint8_t spi_sck_pin,
           uint8_t cs_pin, bool init_spi_hardware)
{

  _spidev = new MySPI(spi_hw,spi_pico_pin,spi_poci_pin,spi_sck_pin,cs_pin,init_spi_hardware);
...
bool Adafruit_BME680::begin(uint8_t addr, bool initSettings) {
  ...
  if (initSettings) {
    ...
    setTemperatureOversampling(BME68X_OS_16X);
...
static int8_t spi_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t len,
                       void *intf_ptr) {
  MySPI *_dev = (MySPI *)intf_ptr;

  reg_addr |= 0x80;

  _dev->read(reg_addr,reg_data,len);
...
static int8_t spi_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t len,
  ...
  _dev->write(reg_addr,reg_data,len);
